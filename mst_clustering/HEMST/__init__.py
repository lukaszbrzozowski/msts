...
class RecHEMST:
    ...
    def _map_to_representants(self):
        while self.n_c > self.desired_k:
            components = [[self.labels[i, 0] for i in range(self.labels.shape[0]) if self.labels[i, 1] == j] for j in np.unique(self.labels[:, 1])]
            current_mapping = {}
